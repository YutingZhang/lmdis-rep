...
download_model() 
{
    ...
    TMP_MODEL_TAR_BALL="${TMP_DIR}/aflw_pretrained_results.tar.gz"

    MODEL_URL="http://files.ytzhang.net/lmdis-rep/release-v1/aflw/aflw_pretrained_results.tar.gz"
    ...
    mv "${TMP_DIR}/aflw_pretrained_results/aflw_10" "${MODEL_DIR}/aflw_10"
    mv "${TMP_DIR}/aflw_pretrained_results/aflw_30" "${MODEL_DIR}/aflw_30"
...
download_data() 
{
    ...
    TMP_DATA_TAR_BALL="${DATA_DIR}/aflw_data.tar.gz"
    DATA_URL="http://files.ytzhang.net/lmdis-rep/release-v1/aflw/aflw_data.tar.gz"
    ...
    TMP_IMAGE_TAR_BALL="${DATA_DIR}/aflw_images.tar.gz"
    IMAGE_URL="http://files.ytzhang.net/lmdis-rep/release-v1/aflw/aflw_images.tar.gz"
    echo "Downloading images ..."
    wget ${WGET_FLAG} "${IMAGE_URL}" -O "${TMP_IMAGE_TAR_BALL}"
    echo "Uncompressing images ..."
    tar -xzf "${TMP_IMAGE_TAR_BALL}" -C "${DATA_DIR}" 
    mv "${DATA_DIR}/output" "${DATA_DIR}/aflw_images"
