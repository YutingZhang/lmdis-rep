...
SNAPSHOT_ITER=""
SPECIFIC_MODEL_DIR=$(cd `dirname $0`; pwd)'/pretrained_results/cat_10'

TRAIN_GT_FILE=$(cd `dirname $0`; pwd)'/data/cat_data/cat_train_keypoints.mat'
TEST_GT_FILE=$(cd `dirname $0`; pwd)'/data/cat_data/cat_test_keypoints.mat'
...
matlab -nosplash -r "cd('./evaluation');cat_evaluation('$TRAIN_PRED_FILE','$TRAIN_GT_FILE','$TEST_PRED_FILE','$TEST_GT_FILE');exit()"
